...
public class LoanCalcViewController implements Initializable {
	...
	private boolean ValidateData() {
		...
		if(InterestRate.getText().trim().isEmpty()) {
			Alert fail = new Alert(AlertType.ERROR);
			fail.setHeaderText("Missing Data");
			fail.setContentText("Interest Rate Required");
			fail.showAndWait();
			return false;
		}
		LoanAmount.getText(); 
		...
		if(LoanAmounts < 0) {
			Alert fail = new Alert(AlertType.ERROR);
			fail.setHeaderText("Please enter valid Loan Amount");
			fail.setContentText("Loan Amount must be positive double");
			fail.showAndWait();
			return false;
		}
		double interestRates = Double.parseDouble(InterestRate.getText()); 
		if(interestRates < 1 || interestRates > 30) {
			Alert fail = new Alert(AlertType.ERROR);
			fail.setHeaderText("Please enter valid Interest Rate");
			fail.setContentText("Interest rate must be positive double between 1 and 30");
			fail.showAndWait();
			return false;
		}
		if(NbrOfYears.getText().trim().isEmpty()) {
			Alert fail = new Alert(AlertType.ERROR);
			fail.setHeaderText("No Number of years");
			fail.setContentText("Enter non blank positive integer");
			fail.showAndWait();
			return false;
		}
		int numYrs = Integer.parseInt(NbrOfYears.getText());
		if(numYrs < 0) {
			Alert fail = new Alert(AlertType.ERROR);
			fail.setHeaderText("Invalid Number of years");
			fail.setContentText("Enter non blank positive integer");
			fail.showAndWait();
			return false; 
		}
		...
		if(addPay < 0) {
			...
			fail.setContentText("Enter positive double");
			fail.showAndWait();
			return false; 
		}
		double EsAmount = Double.parseDouble(EscrowAmount.getText());
			if(EsAmount < 0) {
				Alert fail = new Alert(AlertType.ERROR);
	...
	private void btnCalcLoan(ActionEvent event) {
		...
		lblInterestSaved.setText(String.valueOf(loanNoExtra.getTotalInterest() -loanExtra.getTotalInterest()));
